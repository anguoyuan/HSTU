...
#         # print("float_mask:",float_mask.shape)
#         # print("float_mask.sum():",float_mask.sum())
...
#     # def __init__(self, dim_per_token, all_dim, num_experts, inner_dim, ffn_layers, T, k, dropout_p):
#     #     super().__init__()
#     def __init__(self, model_cfg: Dict, common_hp: Dict, model_cls_dict: Dict):
#         super().__init__(model_cfg=model_cfg, common_hp=common_hp, model_cls_dict=model_cls_dict)
#         dim_per_token = model_cfg[Const.HP].get("dim_per_token")
...
#        # self.input_proj = nn.Linear(x_input_dim, all_dim, bias=True)
...
#         results = []
#    #     x = self.input_proj(x)
#         if enable_jagged_ops:
#             rm_in = self.input_model(x)
#         else:
#             B, N, _ = x.size()
