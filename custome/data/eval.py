...
def process_rank_scores(model, seq_features: SequentialFeatures, save_score_csv_item_cols,
    ...
    eval_group_keys = {
        'aid': seq_features.uid.unsqueeze(-1) * torch.ones_like(supervision_ids)
    }
...
def ag_process_rank_scores(scores, model_input, save_score_csv_item_cols,
    ...
    supervision_ids = model_input["candidate_item_id"]
    ...
    # 收集所有需要放进结果 DataFrame 的字段
    eval_group_keys = {
        'uid': model_input["uid"].unsqueeze(-1) * torch.ones_like(ground_truth[0])
    }
    for key in save_score_csv_item_cols:
    ...
    for key in save_score_csv_user_cols:
        if key not in model_input.keys():
            raise ValueError(key + " not in model_inputs")
        eval_group_keys[key] = model_input[key].unsqueeze(-1) * torch.ones_like(ground_truth[0])
